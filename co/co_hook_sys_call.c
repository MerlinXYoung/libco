...
#include "co_comm.h"
#include "co_inner.h"
#include "co_specific.h"
#include "utils.h"

typedef long long ll64_t;
typedef struct rpchook_s rpchook_t;
struct rpchook_s {
  int user_flag;
...
static close_pfn_t g_sys_close_func =
    NULL; //(close_pfn_t)dlsym(RTLD_NEXT, "close");

static read_pfn_t g_sys_read_func =
    NULL; // (read_pfn_t)dlsym(RTLD_NEXT, "read");
static write_pfn_t g_sys_write_func =
    NULL; // (write_pfn_t)dlsym(RTLD_NEXT, "write");
...
static send_pfn_t g_sys_send_func =
    NULL; // (send_pfn_t)dlsym(RTLD_NEXT, "send");
static recv_pfn_t g_sys_recv_func =
    NULL; //(recv_pfn_t)dlsym(RTLD_NEXT, "recv");

static poll_pfn_t g_sys_poll_func =
    NULL; //(poll_pfn_t)dlsym(RTLD_NEXT, "poll");

static setsockopt_pfn_t g_sys_setsockopt_func = NULL;
// (setsockopt_pfn_t)dlsym(RTLD_NEXT, "setsockopt");
static fcntl_pfn_t g_sys_fcntl_func =
    NULL; // (fcntl_pfn_t)dlsym(RTLD_NEXT, "fcntl");
...
extern int co_poll_inner(co_epoll_t *ctx, struct pollfd fds[], nfds_t nfds,
                         int timeout, poll_pfn_t pollfunc);

static int _pollfd_comp(const struct pollfd *a, const struct pollfd *b) {
  return a->fd - b->fd;
}
typedef int (*void_comp_fn)(const void *, const void *);
int poll(struct pollfd fds[], nfds_t nfds, int timeout) {
  ...
  nfds_t nfds_merge = 0;
  if (nfds > 1) {
    qsort(fds, nfds, sizeof(struct pollfd), (void_comp_fn)_pollfd_comp);
    fds_merge = (struct pollfd *)malloc(nfds * sizeof(struct pollfd));
    int j = 0;
    int i = 0;

    fds_merge[j] = fds[i++];
    for (; i < nfds; ++i) {
      if (fds_merge[j].fd == fds[i].fd)
  ...
  if (nfds_merge == nfds || nfds == 1) {
    ...
    if (ret > 0) {
      ...
      for (size_t i = 0; i < nfds; i++) {
        if (unlikely(fds_merge[j].fd != fds[i].fd)) {
          ++j;
        }
        assert(j < nfds_merge && fds_merge[j].fd == fds[i].fd);
        fds[i].revents = fds_merge[j].revents & fds[i].events;

        // struct pollfd *p =
        //     bsearch(&fds[i], fds_merge, nfds_merge, sizeof(struct pollfd),
        //             (void_comp_fn)_pollfd_comp);
        // if (p) {
        //   fds[i].revents = p->revents & fds[i].events;
        // }
      }
    }
  }
...
typedef struct co_sysenv_t co_sysenv_t;
struct co_sysenv_t {
  char *name;
  char *value;
};
typedef struct co_sysenv_arr_t co_sysenv_arr_t;
struct co_sysenv_arr_t {
  co_sysenv_t *data;
  size_t cnt;
};
static co_sysenv_arr_t *dup_co_sysenv_arr(co_sysenv_arr_t *arr) {
  co_sysenv_arr_t *lp = (co_sysenv_arr_t *)calloc(sizeof(co_sysenv_arr_t), 1);
  if (arr->cnt) {
    lp->data = (co_sysenv_t *)calloc(sizeof(co_sysenv_t) * arr->cnt, 1);
    lp->cnt = arr->cnt;
    memcpy(lp->data, arr->data, sizeof(co_sysenv_t) * arr->cnt);
  }
  return lp;
}

static int co_sysenv_comp(const void *a, const void *b) {
  return strcmp(((co_sysenv_t *)a)->name, ((co_sysenv_t *)b)->name);
}
static co_sysenv_arr_t g_co_sysenv = {0};

void co_set_env_list(const char *name[], size_t cnt) {
  if (g_co_sysenv.data) {
    return;
  }
  g_co_sysenv.data = (co_sysenv_t *)calloc(1, sizeof(co_sysenv_t) * cnt);
  ...
  if (g_co_sysenv.cnt > 1) {
    qsort(g_co_sysenv.data, g_co_sysenv.cnt, sizeof(co_sysenv_t),
          co_sysenv_comp);
    co_sysenv_t *lp = g_co_sysenv.data;
    co_sysenv_t *lq = g_co_sysenv.data + 1;
...
int setenv(const char *n, const char *value, int overwrite) {
  ...
  if (co_is_enable_sys_hook() && g_co_sysenv.data) {
    ...
    if (self) {
      ...
      co_sysenv_arr_t *arr = (co_sysenv_arr_t *)(self->pvEnv);

      co_sysenv_t name = {(char *)n, 0};

      co_sysenv_t *e = (co_sysenv_t *)bsearch(&name, arr->data, arr->cnt,
...
int co_gethostbyname_r(const char *__restrict name,
                       ...
                       struct hostent **__restrict __result,
                       int *__restrict __h_errnop) {
  static __thread co_mutex_t *tls_leaky_dns_lock = NULL;
  if (tls_leaky_dns_lock == NULL) {
    tls_leaky_dns_lock =
        malloc(sizeof(*tls_leaky_dns_lock)); // new clsCoMutex();
    co_mutex_init(tls_leaky_dns_lock);
  }
  // clsSmartLock auto_lock(tls_leaky_dns_lock);
  int ret = 0;
  co_mutex_lock(tls_leaky_dns_lock);
  ret = g_sys_gethostbyname_r_func(name, __result_buf, __buf, __buflen,
                                   __result, __h_errnop);
  co_mutex_unlock(tls_leaky_dns_lock);
  return ret;
}
...
typedef struct res_state_wrap res_state_wrap;
struct res_state_wrap {
  struct __res_state state;
};
// CO_ROUTINE_SPECIFIC(res_state_wrap, __co_state_wrap);
CO_ROUTINE_SPECIFIC1(res_state_wrap, __co_state_wrap);
...
#endif
typedef struct hostbuf_wrap hostbuf_wrap;
struct hostbuf_wrap {
  struct hostent host;
  char *buffer;
  size_t iBufferSize;
...
CO_ROUTINE_SPECIFIC1(hostbuf_wrap, __co_hostbuf_wrap);
...
struct hostent *co_gethostbyname(const char *name) {
  ...
  struct hostbuf_wrap *p = __co_hostbuf_wrap();
#if 1
  if (p->buffer && p->iBufferSize > 1024) {
    free(p->buffer);
    p->buffer = NULL;
  }
  if (!p->buffer) {
    p->buffer = (char *)malloc(1024);
    p->iBufferSize = 1024;
  }

  struct hostent *host = &p->host;
  struct hostent *result = NULL;
  int *h_errnop = &(p->host_errno);

  int ret = -1;
  while ((ret = gethostbyname_r(name, host, p->buffer, p->iBufferSize, &result,
                                h_errnop)) == ERANGE &&
         *h_errnop == NETDB_INTERNAL) {
    free(p->buffer);
    p->iBufferSize = p->iBufferSize * 2;
    p->buffer = (char *)malloc(p->iBufferSize);
  }
#else
